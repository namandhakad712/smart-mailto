# docs

## 0.1.2

### Patch Changes

- Updated dependencies [4b75556]
- Updated dependencies [7143396]
  - @smart-mailto/core@0.3.0
  - @smart-mailto/react@0.2.1

## 0.2.0

### Minor Changes

- 8509bd6: Add versioned install command (npm/yarn/pnpm/bun) alongside existing snippet
- ab338ad: Add privacy-safe product analytics via PostHog to docs and demo (PR #3 / #3 — later removed in a67dc0c)
- 03821c8: Add replace-mailto integration guide (PR #5)

### Patch Changes

- 55c34fb: Resolve Lighthouse accessibility failures — improve contrast ratios and restore focus-visible ring on ThemeToggle (PR #4 / quickwin/lighthouse-task)
- 3277200: Add homepage FAQ section with 8 questions and JSON-LD FAQPage structured data (quickwin/faq-task)
- 31bcbf0: Replace stale homepage dateline and clarify discovery metadata (PR #7)
- 2751fb4: Clarify free pricing in homepage FAQ
- 44812fb: Restore passing docs lint state
- a626568: Format accessibility changes
- b55121b: Clarify copy-address fallback behavior in examples section (PR #10)
- 9578cca: Add package support routes/links (PR #6)
- 77ad9bd: Refresh README discovery links (PR #11)
- e31c6f4, 5d7eadd: Clarify homepage discovery metadata and label provider search field (PR #7, fix/provider-search-label-task)
- 2949b46: Merge fix/provider-search-label changes resolving merge conflicts
- a67dc0c: Remove PostHog analytics integration and posthog-js dependency
