...
buildUrl: (p: MailtoParams) => {
  return 'https://mail.google.com/mail/?view=cm&fs=1&to=' + e(p.to.join(',')) + '&cc=' + e(joinAddresses(p.cc)) + '&bcc=' + e(joinAddresses(p.bcc)) + '&su=' + e(p.subject ?? '') + '&body=' + e(p.body ?? '') + '';
  },
  color: '#EA4335',
  textColor: '#ffffff',
  regions: ['global'],
...
buildUrl: (p: MailtoParams) => {
  return 'mailto:' + p.to.join(',') + '?subject=' + e(p.subject ?? '') + '&body=' + e(p.body ?? '') + (p.cc?.length ? '&cc=' + e(joinAddresses(p.cc)) : '') + (p.bcc?.length ? '&bcc=' + e(joinAddresses(p.bcc)) : '');
  },
  color: '#666666',
  textColor: '#ffffff',
  isNative: true,
